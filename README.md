...
cgi部分我最初的时候完全不理解，整个项目的代码部分cgi有很多都是ai提供的，理论知识不太够，时间也更紧张一些
...
在测试cgi功能的时候，从index.html去跳转页面的时候会需要刷新多次才能让页面正确加载出来，原因是可以开启DEBUG后看到server收到的请求会莫名其妙少一个字符，比如开头的 GET 会少一个 G 变成 ET 导致出现 501 ，不知道是什么原因，但是代码在写CGI的时候已经过去了很久，我也想不到是哪里错了

并且这个现象只在跳转会出问题，可能是一些初始化的问题，因为关于这方面我写的非常粗糙，后续对同一个页面刷新就不会出现这种问题
